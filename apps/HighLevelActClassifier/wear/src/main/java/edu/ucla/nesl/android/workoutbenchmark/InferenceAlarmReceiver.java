...
public class InferenceAlarmReceiver extends BroadcastReceiver {
    ...
    private final static int ALARM_INTERVAL = 1000 * 5; // Millisec * Second * Minute
    private final static int BATTERY_LOG_INTERVAL = 6;
    private static SimpleDateFormat formatForFile = new SimpleDateFormat("yyyyMMdd_HHmmss");
    ...
    public void init() {
        ...
        // Create classifier
        mClassifier = new HighLevelActivityClassifier();
    }
    ...
    public void setAlarm(Context context) {
        ...
        am.setExact(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + ALARM_INTERVAL, pi);
        // Log.i(TAG, "Alarm set.");
    }
    ...
    private void logBatteryPerc(Context context) {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        wl = mPowerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "alarm_wl");
        wl.acquire();

        mContext = context;

        // Log.i(TAG, "InferenceAlarmReceiver received. alarm count=" + alarmCount);
        if (alarmCount % BATTERY_LOG_INTERVAL == 0) {
            logBatteryPerc(context);
        }
        alarmCount++;
        if (alarmCount >= Integer.MAX_VALUE) {
            alarmCount = 0;
        }
        ...
        if (mSensorManager != null) {
            if (accelerometerSensor != null) {
                ...
                // Log.i(TAG, "InferenceAlarmReceiver execution started at " + formatForFile.format(new Date()) + ", type=" + classifierType);
            } else {
    ...
    private void stopRecording() {
        ...
        // Log.i(TAG, "InferenceAlarmReceiver execution finished at " + formatForFile.format(new Date()));
    ...
    private static final ClassifierType classifierType = ClassifierType.CRF;

    // HMM queue
    private static final int HMM_QUEUE_SIZE = 50;
    private static Queue<Integer> resultQueue = new LinkedList<>();
    private static long start = 0;

    private class HighLevelActivityClassifier implements SensorEventListener {
        int count = 0;
        double[] data = new double[200];


        public HighLevelActivityClassifier() {
            super();
        }
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            if (event.sensor.getType() == SENS_ACCELEROMETER) {
                ...
                if (count >= 50) {
                    // Log.i(TAG, "time to reach 50 samples=" + (cur - start));
...
//                }
        ...
        private void performInference(double[] mData) {
            ...
            // Release wl and flush file after each inference
            stopRecording();
        }

        private int findClosestClusterCenter(double[] a, double x) {
            int idx = 0;
