...
Sentence::data(int pos, int col) const {
    const Rep *r = rep();
    if (pos >= 0 && pos < size()) if (col >= 0 && col < r->columns)
...
// else
    ...
    if (verbose)
        cout << " obj=" << obj << " err=" << errors << " (" << misrate << "%)";
    if (tm && verbose)
        cout << " time=" << tm->elapsed() << "s";
    if (verbose)
        cout << endl;

    // __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "obj=%f, err=%d, misrate=%f", obj, errors, misrate);
}


string modelFile = "/sdcard/crf/test_model";
string templateFile = "/sdcard/crf/template";
string trainFile = "/sdcard/crf/train.txt";
string testFile = "/sdcard/crf/train.txt";
double c = 1;
double eta = 0;
int cutoff = 3;
int epochs = 1000;
...
int classify(int *features) {
    // Read features and add to sentence
    int i = 0;
    for (i = 0; i < n; i++) {
        string str(1, features[i]);
        s.push_back(str);
    }

    // Make sure only two windows are in the current sentence
    if (s.size() >= n * 2 + 1) {
        for (i = 0; i < n; i++) {
            s.erase(s.begin());
        }
...
void classify_all() {
    ...
    // Read test file (use the same train file) line-by-line
...
extern "C" {
    ...
    Java_edu_ucla_nesl_android_workoutbenchmark_BGDataCollectionService_classify(JNIEnv *env,
                                                                                 jclass type,
                                                                                 jintArray features_) {
        // __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In classify()");
        jsize len = env->GetArrayLength(features_);
        if (len == n) {
            jint *features = env->GetIntArrayElements(features_, NULL);
            int res = classify(features);
            env->ReleaseIntArrayElements(features_, features, 0);
            return res;
        }
        else {
            __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "error in array length: %d", len);
            return -1;
        }
    }

    JNIEXPORT jint JNICALL
    Java_edu_ucla_nesl_android_workoutbenchmark_BGDataCollectionService_load_1model(JNIEnv *env,
                                                                                    jobject instance) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In load_model()");
        ifstream f(modelFile.c_str());
        f >> crf;
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "done loading model");
        return 1;
    }

    JNIEXPORT jint JNICALL
    Java_edu_ucla_nesl_android_workoutbenchmark_BGDataCollectionService_ndktest(JNIEnv *env,
                                                                                jobject instance) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In ndk_test()");
        ifstream f(modelFile.c_str());
        f >> crf;
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "done loading model");
        loadSentences(testFile.c_str(), crf.getDict(), test);
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "down loading test set");
        ...
        tm.stop();
        // stop tac
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "Done! %f seconds.", tm.elapsed());

        return epochs;
    }

    JNIEXPORT jint JNICALL
    Java_edu_ucla_nesl_android_workoutbenchmark_InferenceAlarmReceiver_load_1model(JNIEnv *env,
                                                                                   jobject instance) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In load_model()");
        ifstream f(modelFile.c_str());
        f >> crf;
    ...
    JNIEXPORT jint JNICALL
    Java_edu_ucla_nesl_android_workoutbenchmark_InferenceAlarmReceiver_classify(JNIEnv *env,
                                                                            jobject instance,
                                                                            jintArray features_) {
        // __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In classify()");
        jsize len = env->GetArrayLength(features_);
    ...
    JNIEXPORT jint JNICALL
    Java_edu_ucla_nesl_android_workoutbenchmark_InferenceAlarmReceiver_ndktest(JNIEnv *env,
                                                                               jobject instance) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "In ndk_test()");
        ifstream f(modelFile.c_str());
        f >> crf;
